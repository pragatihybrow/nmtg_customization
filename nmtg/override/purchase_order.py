    ...
    def validate_supplier_qc(self):
        ...
        for qc in self.custom_supplier_selection_for_qc:
            ...
            if not matching_item:
                frappe.throw(
                    f"Item <b>{qc.item}</b> not found in Purchase Order."
                )
            ...
            try:
                ...
                po_start, po_end = [
                    ...
                    for x in matching_item.custom_nmtg_heat_number.strip().split("-")
                ]

                qc_start, qc_end = [
                    int(x.strip().replace("NK", ""))
                    for x in qc.nmtg_heat_number.strip().replace("\n", "").split("-")
                ]
                ...
                frappe.throw(
                    ...
                    Expected:
                    <b>NK5405 - NK5454</b><br>

                    Entered:
                    <b>{qc.nmtg_heat_number}</b>
                    """
                )
            ...
            if not (po_start <= qc_start <= po_end and qc_end <= po_end):
                ...
                frappe.throw(
                    ...
                    PO Range:
                    <b>{matching_item.custom_nmtg_heat_number}</b><br>
            ...
            for old_start, old_end, old_range in existing:
                ...
                if overlap:
                    ...
                    frappe.throw(
                        ...
                        New Range:
                        <b>
                        NK{next_available} - NK{expected_end}
                        </b>
                        """
                    )
